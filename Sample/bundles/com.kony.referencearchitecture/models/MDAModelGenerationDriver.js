 ...
 */
 konyModelCreateDirectory = function (path, overwrite) {
    if(overwrite)
    {
...
*/
konyModelDeleteUnwantedObjectServicesFolders = function(directoryPath, requiredList){
...
konyModelGenerateModels = function (metadataMap, targetDirectoryForModels) {
    ...
    try {
        for (var i = 0; i < noOfObjectSvc; i++) {
            ...
            konyModelCreateDirectory(path.resolve(targetDirectoryForModels, objectSvcName),true);
            ...
            for (var j = 0; j < noOfObjects; j++) {
                ...
                konyModelCreateDirectory(filePath,true);
        ...
        konyModelDeleteUnwantedObjectServicesFolders(targetDirectoryForModels, view.objectsvc_meta);

        // Saving original schema for RepoConfig.js
        var tempSchemaView = schemaView;

        schemaView = '{"objectsvc_meta":[' + schemaView + "]}";
        var objectsvc_meta = JSON.parse(schemaView);
        var objectServiceJSON = createObjectServiceJSONTemplate(objectsvc_meta);
        const data = {"data":objectServiceJSON};

        modelData = codeGenerationEngine.call(konyModelfetchRenderedData, path.resolve(__dirname, "MDARepoManagerConfig"), data);
...
/**
 * Create a JSON template using objectService_meta data.
 *
 * @param objectsvc_meta     contains object services along with objects
 * @return {object}          modified tableView
 */
createObjectServiceJSONTemplate = function(objectsvc_meta){
    var resultObjSvc = "";
    var resultObj = "";
    var objectServices = [];
    var schema = objectsvc_meta["objectsvc_meta"];
    for(var index in schema){
        if(!objectServices.includes(schema[index]["db"])){
            objectServices.push(schema[index]["db"]);
        }
    }

    for(var index in objectServices) {
        resultObj = "";
        var objSvc = objectServices[index];
        for(var schemaIndex in schema){
            if(schema[schemaIndex]["db"].localeCompare(objSvc) === 0){
                resultObj += codeGenerationEngine.call(konyModelfetchRenderedData, path.resolve(__dirname, "RepoManagerConfigObjectTemplate"), schema[schemaIndex]);
            }
        }
        var objSvcSchema = {
            "db":objSvc,
        ...
        resultObjSvc += codeGenerationEngine.call(konyModelfetchRenderedData, path.resolve(__dirname, "ObjectServiceJSONTemplate"), objSvcSchema);
    }
    return resultObjSvc;
};
...
konyModelMasterRepoManagerGenerator = function(sources, destination) {
    ...
    try {
        ...
        var objectsvc_meta = JSON.parse(scheme);
        var objectServiceJSON = createObjectServiceJSONTemplate(objectsvc_meta);
        const data = {"data":objectServiceJSON};
        modelData = codeGenerationEngine.call(konyModelfetchRenderedData, path.resolve(__dirname, "MDARepoManagerConfig"), data);

        konyModelCreateDirectory(destination,false);
